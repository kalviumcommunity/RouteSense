...
const SignUp = () => {
  ...
  const handleSignUp = (e) => {
    e.preventDefault();
    // In a real app, you'd create the account here. For demo, we just go to dashboard.
    navigate('/signin');
  };
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-[#F8FAFC] flex flex-col items-center justify-center font-sans p-12">
      ...
      <div className="flex flex-col items-center gap-6 mb-12">
        ...
        <h2 className="text-4xl font-black tracking-[-0.05em]">Get Started Today.</h2>
        <p className="text-slate-400 text-sm font-bold uppercase tracking-widest leading-relaxed">Join 1,000+ Fleet Managers</p>
      </div>

      <div className="w-full max-w-sm glass-card p-10 flex flex-col gap-6">
        <form onSubmit={handleSignUp} className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <label className="text-[11px] font-bold uppercase text-slate-500 tracking-widest pl-2">Full Name</label>
            <input 
              type="text" 
              required
          ...
          <button type="submit" className="bg-gradient-to-r from-[#4FACFE] to-[#00F2FE] text-black p-4 mt-2 rounded-xl font-black flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform primary-glow">
            Create Account <ArrowRight size={18} />
          </button>
        </form>

        <div className="text-center text-xs text-slate-500 font-bold uppercase tracking-widest mt-4">
          Already have an account? <Link to="/signin" className="text-[#00F2FE] hover:underline">Sign In</Link>
        </div>
      </div>
    </div>
  );
